...
Expression Z3Interface::fromZ3Expr(z3::expr const& _expr)
{
	...
	{
		smtAssert(false, "");
		size_t size = std::stoul(arguments[1].name);
		return Expression::int2bv(arguments[0], size);
	}
	else if (kind == Z3_OP_BV2INT)
	{
		smtAssert(false, "");
		return Expression::bv2int(arguments[0], false);
	}
...
vector<SortPointer> Z3Interface::fromZ3Sort(z3::sort_vector const& _sorts)
{
	...
	for (auto const& sort: _sorts)
		sorts.push_back(fromZ3Sort(sort));
	return sorts;
}
