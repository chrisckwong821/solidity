...
string Predicate::formatSummaryCall(vector<smtutil::Expression> const& _args) const
{
	...
	vector<smtutil::Expression>::const_iterator first = _args.begin() + 5 + static_cast<int>(stateVars->size());
	vector<smtutil::Expression>::const_iterator last = first + static_cast<int>(fun->parameters().size());
...
vector<optional<string>> Predicate::summaryPostInputValues(vector<smtutil::Expression> const& _args) const
{
	...
	vector<smtutil::Expression>::const_iterator first = _args.begin() + 5 + static_cast<int>(stateVars->size()) * 2 + static_cast<int>(inParams.size()) + 1;
	vector<smtutil::Expression>::const_iterator last = first + static_cast<int>(inParams.size());
...
vector<optional<string>> Predicate::summaryPostOutputValues(vector<smtutil::Expression> const& _args) const
{
	...
	vector<smtutil::Expression>::const_iterator first = _args.begin() + 5 + static_cast<int>(stateVars->size()) * 2 + static_cast<int>(inParams.size()) * 2 + 1;
